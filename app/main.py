...
# NEW: Preview only
@app.post("/enrich/person/preview")
...
# NEW: Confirm with specific QID
@app.post("/enrich/person/confirm")
def confirm_person(payload: EnrichName):
...
class EnrichNamesList(BaseModel):
    names: list[str]
@app.post("/enrich/persons/all-from-db")
...
@app.post("/enrich/persons/all-from-db-auto")
def enrich_all_auto():
    """
    Auto-loop SEMUA person dari Neo4j (batch by batch).
